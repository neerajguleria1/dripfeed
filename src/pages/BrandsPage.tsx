...
export default function BrandsPage() {
  ...
  return (
    <div className="min-h-screen text-[#0F0F1A] flex flex-col relative">

      {/* \u2500\u2500 Sticky Header \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500 */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#C9A96E]/60 backdrop-blur-[12px] shadow-[0_4px_20px_rgba(5,31,69,0.06)]" style={{backgroundBlendMode: 'luminosity'}}>
        <div className="max-w-[1200px] mx-auto flex items-center justify-between px-6 lg:px-8 py-4">
          <div className="flex items-center gap-6">
            <Link to="/" className="inline-flex items-center gap-2 text-[#0F0F1A]/60 hover:text-[#0F0F1A] transition-colors text-sm">
      ...
      <section ref={heroRef} className="relative z-10 px-6 lg:px-8 pt-40 pb-20 md:pt-48 md:pb-28">
        <div className="max-w-[800px] mx-auto text-center">
          ...
          <p className="hero-subtitle text-[#0F0F1A]/50 text-lg sm:text-xl max-w-[600px] mx-auto leading-relaxed mb-10">
          ...
          <div className="hero-cta flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="#problems" className="inline-flex items-center gap-2 text-[#0F0F1A]/50 hover:text-[#0F0F1A] text-sm font-medium transition-colors">
      ...
      <section id="problems" className="relative z-10 px-6 lg:px-8 py-20 md:py-28">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full border border-[#0F0F1A]/8 bg-white/80 text-[10px] font-mono tracking-[0.2em] uppercase text-[#0F0F1A]/50 mb-4">The Problem</span>
            <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold tracking-[-0.02em] text-[#0F0F1A]">The problem for fashion merchants today</h2>
          </div>
            ...
            {problems.map((problem, i) => {
              ...
              return (
                <div key={problem.title}
                  className={`problem-card group relative rounded-2xl p-7 cursor-pointer transition-all duration-500 ${isActive ? 'bg-[#0F0F1A] text-white shadow-2xl shadow-[#0F0F1A]/15 -translate-y-1 scale-[1.01]' : 'bg-white/70 backdrop-blur-sm border border-white/50 hover:bg-white/90 hover:-translate-y-1 hover:shadow-lg hover:shadow-[#0F0F1A]/5'}`}
                  onMouseEnter={() => setActiveProblem(i)}>
                  <div className="flex items-start gap-4">
                      <div className={`flex items-center justify-center w-12 h-12 rounded-xl shrink-0 transition-all duration-300 ${isActive ? 'bg-white/15 text-white' : 'bg-white text-[#0F0F1A] border border-[#0F0F1A]/8 shadow-sm'}`}>
                      <Icon className="w-6 h-6" />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-xl font-semibold mb-2">{problem.title}</h3>
                      <p className={`text-sm leading-relaxed ${isActive ? 'text-white/70' : 'text-[#0F0F1A]/50'}`}>{problem.desc}</p>
                    </div>
                  </div>
                  <div className={`mt-5 pt-5 border-t flex items-baseline gap-3 transition-all duration-300 ${isActive ? 'border-white/15' : 'border-[#0F0F1A]/5'}`}>
                    <span className="font-serif text-4xl font-bold">{problem.stat}</span>
                    <span className={`text-xs uppercase tracking-wider ${isActive ? 'text-white/50' : 'text-[#0F0F1A]/30'}`}>{problem.statLabel}</span>
                  </div>
                </div>
              );
            })}
      ...
      <section className="relative z-10 px-6 lg:px-8 py-20 md:py-28">
        <div className="max-w-[1200px] mx-auto">
          <div className="relative rounded-3xl bg-[#0F0F1A] p-10 md:p-16 text-white overflow-hidden">
            <div className="absolute top-0 right-0 w-96 h-96 bg-white/5 rounded-full blur-[80px]" />
            <div className="absolute bottom-0 left-0 w-72 h-72 bg-[#C9A96E]/10 rounded-full blur-[60px]" />
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((feature) => {
              ...
              return (
                <div key={feature.title} className="feature-item group relative rounded-2xl bg-white/70 backdrop-blur-sm border border-white/50 p-6 transition-all duration-500 hover:bg-white/90 hover:shadow-lg hover:shadow-[#0F0F1A]/5 hover:-translate-y-1">
                  <div className="inline-block px-2.5 py-1 rounded-full bg-white border border-[#0F0F1A]/8 text-[9px] font-mono tracking-wider uppercase text-[#0F0F1A]/50 mb-4">{feature.tag}</div>
                  <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-[#0F0F1A]/8 mb-4 group-hover:scale-110 transition-transform duration-300 shadow-sm">
                    <Icon className="w-5 h-5 text-[#0F0F1A]" />
                  </div>
                  <h3 className="text-lg font-bold text-[#0F0F1A] mb-2">{feature.title}</h3>
                  <p className="text-[#0F0F1A]/50 text-sm leading-relaxed">{feature.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="relative z-10 px-6 lg:px-8 py-20 md:py-28">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full border border-[#0F0F1A]/8 bg-white/80 text-[10px] font-mono tracking-[0.2em] uppercase text-[#0F0F1A]/50 mb-4">Media Opportunities</span>
            <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold tracking-[-0.02em] text-[#0F0F1A]">Amplify your reach</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {mediaFeatures.map((feature) => {
              const Icon = feature.icon;
              return (
                <div key={feature.title} className="media-card group relative rounded-2xl bg-white/70 backdrop-blur-sm border border-white/50 overflow-hidden transition-all duration-500 hover:shadow-xl hover:shadow-[#0F0F1A]/5 hover:-translate-y-1">
                  <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#0F0F1A] to-[#1a4a8b] opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <div className="p-8">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-[#0F0F1A] text-white">
                        <Icon className="w-5 h-5" />
                      </div>
                      <h3 className="text-xl font-bold text-[#0F0F1A]">{feature.title}</h3>
                    </div>
                    <p className="text-[#0F0F1A]/60 text-sm leading-relaxed mb-4">{feature.desc}</p>
                    <div className="pt-4 border-t border-[#0F0F1A]/5">
                      <p className="text-[#0F0F1A]/40 text-xs leading-relaxed">{feature.benefit}</p>
                    </div>
                  </div>
                </div>
              );
      ...
      <section id="steps" className="relative z-10 px-6 lg:px-8 py-20 md:py-28">
        <div className="max-w-[800px] mx-auto">
          <div className="text-center mb-16">
            <span className="inline-block px-4 py-1.5 rounded-full border border-[#0F0F1A]/8 bg-white/80 text-[10px] font-mono tracking-[0.2em] uppercase text-[#0F0F1A]/50 mb-4">Get Started</span>
            <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold tracking-[-0.02em] text-[#0F0F1A]">How to get started</h2>
          </div>
          <div className="relative">
            <div className="absolute left-[27px] top-0 bottom-0 w-px bg-[#0F0F1A]/8" />
            <div className="space-y-4">
              {steps.map((step) => {
                const Icon = step.icon;
                return (
                  <div key={step.num} className="step-item group relative flex items-start gap-6 rounded-2xl bg-white/70 backdrop-blur-sm border border-white/50 p-6 hover:bg-white/90 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#0F0F1A]/5">
                    <div className="relative z-10 flex items-center justify-center w-14 h-14 rounded-2xl bg-[#0F0F1A] text-white shrink-0 group-hover:scale-110 transition-transform duration-300">
                      <Icon className="w-6 h-6" />
                    </div>
                    <div className="flex-1 pt-2">
                      <span className="text-[10px] font-mono text-[#0F0F1A]/30 tracking-wider">STEP {step.num}</span>
                      <h3 className="text-lg font-bold text-[#0F0F1A] mb-1">{step.title}</h3>
                      <p className="text-[#0F0F1A]/50 text-sm">{step.desc}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {showBackToTop && (
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/90 backdrop-blur-sm border border-[#C9A96E]/30 shadow-lg shadow-[#0F0F1A]/5 text-[#0F0F1A]/60 hover:text-[#0F0F1A] text-xs font-medium transition-all duration-300 hover:shadow-xl hover:bg-white hover:border-[#C9A96E]/60">
