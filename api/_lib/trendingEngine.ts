...
/**
 * Runs a single MongoDB aggregation over AnalyticsEvent for the given window.
 * Returns raw signal counts per canonicalId — scoring is applied in JS.
 */
async function aggregateSignals(
  from: Date,
  category?: string,
): Promise<Map<string, { title: string; platform?: string; signals: TrendingProduct['signals'] }>> {
  const AnalyticsEvent = (await import('./models/AnalyticsEvent.js')).default;

  // Map event names → signal keys
  ...
  const result = new Map<string, { title: string; platform?: string; signals: TrendingProduct['signals'] }>();

  for (const row of rows) {
    const signals: TrendingProduct['signals'] = {
      views: 0, compareClicks: 0, wishlistAdds: 0, affiliateClicks: 0, priceAlerts: 0,
    };
    for (const { event, count } of row.events) {
      const key = EVENT_SIGNAL_MAP[event];
      if (key) signals[key] += count;
    }
    result.set(row._id, {
